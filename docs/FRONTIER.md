...
| **BFT view-change liveness** [OPEN-BFT-01] | Single-slot finality with view-change when a proposer fails; **safety is sound** — votes lock per-height so no two conflicting blocks commit, and the `N−(N−1)/3` quorum makes conflicting commits impossible (TIER 0). **Common wedge now FIXED:** proposals are content-deterministic (`Ledger.nextBlockTs` derives ts from chain state instead of the wall clock; mempool selection is in-order), so a view-change re-proposal of the same mempool is byte-identical and hashes the same — the validators that voted the view-0 block re-vote the view-1 block, so the dominant "proposer crash with a converged mempool" case finalizes instead of wedging. | **Residual (still open):** a vote locks per-**height**, not per-(height,view), and is never released, so if proposers disagree on content (divergent mempool, or a Byzantine proposer) the later-view block differs and the locked voters still reject it → the height can wedge. Safely closing this needs PBFT-style **view-change certificates** (re-propose the highest-voted block, justified by a quorum of view-change messages). Per project philosophy (no hand-rolled safety-critical code — same stance as threshold-ML-DSA below), this is **not rolled in-house**: the target behaviour + safety argument + adversarial acceptance tests are specified in [`docs/CONSENSUS-VIEWCHANGE.md`](CONSENSUS-VIEWCHANGE.md) as the contract for a **vetted/audited BFT engine** (e.g. BFT-SMaRt) or a formally-verified implementation to satisfy when adopted. A naive hand-rolled lock-release would break safety (the view-0 block could have committed on another partition). Both the interim fix and the residual are pinned by [`VoteLockWedgeTest`](../src/test/java/com/phantomchain/debug/VoteLockWedgeTest.java). |
