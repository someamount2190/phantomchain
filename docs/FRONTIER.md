...
| **BFT view-change liveness** [OPEN-BFT-01] | Single-slot finality with view-change when a proposer fails; **safety is sound** — votes lock per-height so no two conflicting blocks commit, and the `N−(N−1)/3` quorum makes conflicting commits impossible (TIER 0). | A vote locks per-**height**, not per-(height,view), and is never released; the block hash commits to wall-clock `ts`, so a later-view proposal has a different hash that every view-0 voter rejects. After a **sub-quorum partial vote that then stalls** (proposer crash / partition), no later view can finalize that height — a **liveness wedge**, reproduced deterministically by [`VoteLockWedgeTest`](../src/test/java/com/phantomchain/debug/VoteLockWedgeTest.java) (N=4, quorum 3). Reachable for any committee (quorum ≈⅔ > (N+1)/2). The fix is PBFT-style **view-change certificates**; a naive lock-release would break safety (the view-0 block could have committed on another partition), so it is a real protocol addition, not a patch. |
