...
**Run** — one process per node, each via harness `run_in_background`. Only the seed is shared config; the rest is discovered:
```powershell
# NodeMain <index> <port> <selfAddr> <seedAddr> [dataDir]
java -cp "$st\node\out;$st\lib\*" com.phantomchain.debug.NodeMain 0 9091 127.0.0.1:9091 127.0.0.1:9091 "$st\node\node-data3"
java -cp "$st\node\out;$st\lib\*" com.phantomchain.debug.NodeMain 1 9092 127.0.0.1:9092 127.0.0.1:9091 "$st\node\node-data3"
java -cp "$st\node\out;$st\lib\*" com.phantomchain.debug.NodeMain 2 9093 127.0.0.1:9093 127.0.0.1:9091 "$st\node\node-data3"
```
node0 is its own seed (bootstrap); node1/node2 learn the full membership over the network. Validator keys/genesis are deterministic. State persists to `node-data3\node-<i>.json`. (Deleting these is sandbox-blocked — use a fresh data-dir name to reset.)
...
- **view-change**: kill the proposer for the next height (`Get-CimInstance Win32_Process … '*NodeMain 2 *'` → `Stop-Process`), `/submit` → after ~3 s another node proposes at view 1 and commits with the remaining 2-of-3 (its stdout logs `VIEW-CHANGE`). Verified.
...
**Still NOT built / NOT real mining:** the QC is a **multisig, not a true threshold signature** — several separate ML-DSA signatures, not one aggregate. Byzantine accountability is **partial**: it punishes the classic *equivocation* (double-sign) fault with persistent-vote prevention + slashing, but it is not a full BFT safety proof — other adversarial behaviors (vote withholding to stall, invalid-block spam, long-range/eclipse attacks) aren't handled, and a slashed node is excluded but not network-banned. Cross-machine is supported by design (host:port) but only **tested on one host**. No Reed-Solomon sharding, no rewards, no real token economics. The "prop mesh server" remains a localhost mock.
